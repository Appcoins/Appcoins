...
uint256 hashGenesisBlock("0x60b7e3d142cdc8c7e1fa33893effa9bb98fec85e675e5ab71b987ba580d2a0b4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Appcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Appcoin Signed Message:\n";
...
{
    // Appcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    int64 nSubsidy = 1 * COIN;
       ...
       if(nHeight == 2)  
        {   
                nSubsidy = 33840200 * COIN;  
        }
        else if(nHeight == 3)      
        {
                nSubsidy = 135360000 * COIN;
        }
        else if(nHeight < 50)      
        {
                nSubsidy = 0 * COIN;
        ...
        else if(nHeight < 5200000)      
        {
                nSubsidy = 500 * COIN;
        }
         else if(nHeight < 5600000)      
        {
                nSubsidy = 250 * COIN;
        }
        else if(nHeight < 6000000)      
        {
         ...
         else if(nHeight < 6400000)      
        {
                nSubsidy = 150 * COIN;
        }
        else if(nHeight < 6800000)      
        {
                nSubsidy = 100 * COIN;
        } 
...
static const int64 nTargetTimespan = 20 * 60; // Appcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Appcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Appcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Appcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Appcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x60b7e3d142cdc8c7e1fa33893effa9bb98fec85e675e5ab71b987ba580d2a0b4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Major SSl code flaw patched in ios. Go to Fail lol Go to Fail";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1394358982;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2852241;
       ...
       if (fTestNet)
        ...
        assert(block.hashMerkleRoot == uint256("0x8c7dc6e137d451a2edd3a914f471519cba0787fb11cf6b33f71cb7e599fd77c2"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Appcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AppcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AppcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AppcoinMiner(CWallet *pwallet)
{
    printf("AppcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("appcoin-miner");
    ...
    try { loop {
        ...
        printf("Running AppcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AppcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AppcoinMiner, pwallet));
}
